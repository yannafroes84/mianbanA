...
func updateServices(req updateServicesRequest) error {
	...
	type oldServiceInfo struct {
		name    string
		service service.Service
		config  *config.ServiceConfig
	}

	var rollbackList []oldServiceInfo
	...
	for _, serviceConfig := range req.Data {
		...
		svc, err := parser.ParseService(&serviceConfig)
		if err != nil {
			for _, rb := range rollbackList {
				if rsvc, rerr := parser.ParseService(rb.config); rerr == nil {
					registry.ServiceRegistry().Register(rb.name, rsvc)
					go rsvc.Serve()
				}
			}
			return errors.New("create service " + name + " failed: " + err.Error())
		...
		rollbackList = append(rollbackList, oldServiceInfo{
			name:    name,
			service: old,
			config:  oldConfig,
		})
