...
public class ResetFlowAsync {
    ...
    private void cleanupCryptoCache() {
        try {
            List<Node> activeNodes = nodeService.list(new QueryWrapper<Node>().eq("status", 1).isNotNull("secret"));
            Set<String> activeSecrets = activeNodes.stream()
                    .map(Node::getSecret)
                    .collect(Collectors.toSet());
            WebSocketServer.cleanupCryptoCache(activeSecrets);
        } catch (Exception e) {
            log.info("清理加密缓存失败", e);
        }
    }
