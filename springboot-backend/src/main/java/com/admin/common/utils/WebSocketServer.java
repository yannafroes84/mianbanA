...
public class WebSocketServer extends TextWebSocketHandler {
    ...
    public static void cleanupCryptoCache(Set<String> activeSecrets) {
        cryptoCache.keySet().retainAll(activeSecrets);
    }

    public static AESCrypto getCryptoForSecret(String secret) {
        return cryptoCache.computeIfAbsent(secret, k -> new AESCrypto(k));
    }
    ...
    public static CompletableFuture<GostDto> send_msg_async(Long node_id, Object msg, String type) {
        WebSocketSession nodeSession = nodeSessions.get(node_id);

        CompletableFuture<GostDto> errorFuture = new CompletableFuture<>();

        if (nodeSession == null) {
            log.info("发送消息失败：节点 {} 不在线或会话不存在", node_id);
            GostDto result = new GostDto();
            result.setMsg("节点不在线");
            errorFuture.complete(result);
            return errorFuture;
        }
        ...
        try {
            ...
            future.orTimeout(10, TimeUnit.SECONDS).exceptionally(ex -> {
                ...
                if (ex instanceof java.util.concurrent.TimeoutException) {
                    ...
                    result.setMsg("发送消息失败: " + ex.getMessage());
                    log.info("发送消息到节点 {} 失败: {}", node_id, ex.getMessage());
                }
                return result;
            });
    ...
    public static GostDto send_msg(Long node_id, Object msg, String type) {
        try {
            return send_msg_async(node_id, msg, type).get(10, TimeUnit.SECONDS);
        } catch (Exception e) {
            GostDto result = new GostDto();
            if (e instanceof java.util.concurrent.TimeoutException) {
                result.setMsg("等待响应超时");
            } else if (e instanceof java.util.concurrent.ExecutionException) {
                result.setMsg("发送消息失败: " + e.getCause().getMessage());
            } else {
                result.setMsg("发送消息失败: " + e.getMessage());
            }
            return result;
